...
function nameOf(
  ...
  if (num == null) return fallback ?? (lang === "en" ? EN_FALLBACK_LEADER : "선두마");
  return map.get(num)?.name ?? (lang === "en" ? enHorseNo(num) : `${num}번`);
}
...
export function buildRaceCommentary(
  ...
  const lang: Lang = ctx.lang ?? "ko";
  const en = lang === "en";

  /** 영어 문장 생성기에 넘길 공통 값 — 한국어 경로에서는 호출되지 않는다. */
  const enCtx = () => ({
    distance: ctx.distance,
    trackWord: EN_TRACK[ctx.track] ?? "turf",
    leaderName: nameOf(ctx.entrantMap, leaderNum(ctx), lang),
    secondName: nameOf(ctx.entrantMap, ctx.keyframe.frame1.ranks[1], lang),
    thirdName: nameOf(ctx.entrantMap, ctx.keyframe.frame1.ranks[2], lang),
    pickedName: ctx.pickedNumber
      ? nameOf(ctx.entrantMap, ctx.pickedNumber, lang)
      : null,
    paceWord: EN_PACE[ctx.entrantMap.get(leaderNum(ctx) ?? 0)?.pace ?? ""] ?? "",
    overtakes: ctx.overtakes,
    photoFinish: ctx.photoFinish,
  });

  if (!ctx.started) {
    if (en) return { text: pick(EN.preStart(enCtx())) };
    return {
      text: pick([
  ...
  if (ctx.finished) {
    if (ctx.dnf) {
      if (ctx.dnfReason === "interference") {
        return {
          text: en ? EN.dnfInterference : "앞 다툼에 휘말려 기권! 간섭 사고입니다.",
          urgent: true,
        };
      }
      return {
        text: en ? EN.dnfFall : "아아, 낙마! 안타까운 기권 처리입니다.",
        urgent: true,
      };
    }
    ...
    if (place === 1) {
      return {
        text: en ? EN.win : "우승! 환호가 터집니다! 멋진 질주였습니다!",
        urgent: true,
      };
    }
    if (place <= 3) {
    ...
    return {
      text: en
        ? pick(EN.plainFinish(enPlace(place)))
        : pick([
  ...
  const leaderName = nameOf(ctx.entrantMap, leader, lang);
  ...
  if (ctx.raceEvents?.length) {
    for (const ev of ctx.raceEvents) {
      ...
      if (ev.type === "interference") {
        const names = ev.horses
          .map((n) => nameOf(ctx.entrantMap, n, lang))
          .join(", ");
        return {
          text: en
            ? pick(EN.interference(names))
            : pick([
      ...
      if (ev.type === "fall" && ev.horses.length === 1) {
        const n = ev.horses[0]!;
        const fallen = nameOf(ctx.entrantMap, n, lang);
        return {
          text: en ? EN.fall(fallen) : `${fallen} 낙마! 기권 처리됩니다!`,
          urgent: true,
        };
      }
    }
  ...
  if (opts?.overtake || (ctx.keyframe.frame1.overtakes && ctx.keyframe.t > 0.4)) {
    return {
      text: en
        ? pick(EN.overtake(enCtx()))
        : pick([
            `역전! ${leaderName}이 앞서 나섭니다!`,
            `순위가 바뀝니다! ${leaderName}, 선두 등극!`,
            `치열합니다! ${nameOf(ctx.entrantMap, second, lang)}를 제치고 ${leaderName}이 1위!`,
  ...
  if (pickedName && pRank <= 3 && ctx.raceProgress > 0.15) {
    if (pRank === 1 && Math.random() < 0.35) {
      return {
        text: en
          ? pick(EN.pickedLeading(pickedName))
          : pick([
    ...
    if (pRank === 2 && Math.random() < 0.25) {
      return {
        text: en
          ? EN.pickedSecond(pickedName)
          : `예상 ${pickedName}, 2위 추격! 한 번에 역전할 각입니다!`,
        urgent: false,
      };
    }
  }
    ...
    return {
      text: en
        ...
        : pick([
            ...
            `${leaderName} 선두, ${nameOf(ctx.entrantMap, second, lang)}가 바짝 추격합니다!`,
            `첫 코너 접근! ${leaderName}—${nameOf(ctx.entrantMap, second, lang)}—${nameOf(ctx.entrantMap, third, lang)} 순!`,
  ...
  if (phase === "middle") {
    return {
      text: en
        ? pick(EN.middle(enCtx()))
        : pick([
            `중반전! ${leaderName}이 리듬을 탔습니다. ${nameOf(ctx.entrantMap, second, lang)} 추격 지속!`,
            `순위 변동 없이 ${leaderName} 주도, 뒤는 한 치의 양보 없습니다!`,
            `${ctx.distance}m 중거, ${leaderName}이 페이스 메이킹 중!`,
            `2번째 코너! ${leaderName} 안쪽 라인, ${nameOf(ctx.entrantMap, second, lang)} 바깥에서 압박!`,
          ]),
    };
  }
    ...
    return {
      text: en
        ...
        : pick([
            ...
            `3위 ${nameOf(ctx.entrantMap, third, lang)}까지 박빙! 승부는 이제부터입니다!`,
  ...
  if (phase === "stretch") {
    return {
      text: en
        ? pick(EN.stretch(enCtx()))
        : pick([
            `막판 직선! ${leaderName}—${nameOf(ctx.entrantMap, second, lang)} 맞대결!`,
            `결승선이 보입니다! ${leaderName}, 끝까지 전력 질주!`,
            `여기서 갈립니다! ${nameOf(ctx.entrantMap, second, lang)} 막판 추입 시도!`,
            ctx.photoFinish
  ...
  return {
    text: en
      ? pick(EN.finish(enCtx()))
      : pick([
          `결승선 앞! ${leaderName}, 우승을 향해 질주!`,
          `마지막 한 걸음! ${leaderName}—${nameOf(ctx.entrantMap, second, lang)} 초접전!`,
          "골인 임박! 관중석이 들썩입니다!",
        ]),
    urgent: true,
  };
