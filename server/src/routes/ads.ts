...
adsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (adToken.startsWith("dev-")) {
      ...
      // dev-/ssv: 접두사가 아닌 토큰은 그 무엇도 검증할 수단이 없다 — 예전엔 이
      // 분기를 그냥 통과시켰는데, 그러면 세션만 있으면(데모 로그인은 항상 열려
      // 있음) 임의 문자열로 광고 시청 없이 매일 한도까지 보상을 받을 수 있었다.
      // 토스 인앱광고용 서버측 검증(SSV 상당)이 열리기 전까지는 거부한다.
      res.status(403).json({ ok: false, message: "유효하지 않은 광고 토큰이에요." });
      return;
    }
